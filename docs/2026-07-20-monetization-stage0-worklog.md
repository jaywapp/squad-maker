...
> `url`에 `#s=` 공유 스냅샷이 그대로 포함되는 문제를 놓쳤다.
> 수정 내용은 [리뷰 지적 수정 내역](./2026-07-20-monetization-stage0-review-fixes.md) §1 참고.
...
이는 배포 차단 테스트가 네트워크 조건에 따라 흔들린다는 실제 문제를 드러냈다
([수정 내역](./2026-07-20-monetization-stage0-review-fixes.md) §3에서 수정).
...
[리뷰 피드백](./2026-07-20-monetization-stage0-worklog-feedback.md)의 지적을 검증하고 수정했다.
검증 근거·재현 결과·코드 변경·테스트 매핑은 별도 문서에 정리했다.

> **전체 내역: [수익화 0단계 리뷰 지적 수정 내역](./2026-07-20-monetization-stage0-review-fixes.md)**

| # | 등급 | 지적 | 검증 결과 | 조치 |
|---|:---:|---|---|---|
| 1 | P1 | 분석 URL에 `#s=` 공유 스냅샷이 실려 나감 | 실재 — 재현 성공 | 3중 방어 + 전송 본문 계약 테스트 |
| 2 | P1 | 스크립트 로드 전 이벤트 유실 | 실재 — 지적보다 심각(결정적 유실) | 이벤트 큐 + 지연·실패 테스트 |
| 3 | P2 | 배포 차단 테스트가 외부 CDN에 의존 | 타당 | 자산 고정 + 외부 요청 차단, CDN은 별도 smoke |
| 4 | P2 | ADR이 동의 배너 불필요를 단정 | 타당 | "법무·설정 검토 후 확정"으로 보정 |
| — | — | (권고) 자동 페이지뷰 추적 끄기 | 미채택 | Gate A에 방문·세션 분모가 필요 |

수정 결과 테스트는 39건에서 **51건**으로 늘었고, 외부 네트워크를 차단한 상태에서 통과한다.
...
초기 구현은 화이트리스트를 만들어 놓고 "구조적으로 강제한다"고 적었지만, 정작 가장 민감한
값(`#s=` 스냅샷)은 우리가 넘기지도 않은 필드를 통해 나가고 있었다. 앞으로 외부 SDK를
붙일 때는 어댑터 단위 검증에서 멈추지 않고 **최종 네트워크 payload를 캡처해서** 확인한다.
